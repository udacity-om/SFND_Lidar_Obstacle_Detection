...
std::vector<typename pcl::PointCloud<PointT>::Ptr> ProcessPointClouds<PointT>::Clustering(typename pcl::PointCloud<PointT>::Ptr cloud, float clusterTolerance, int minSize, int maxSize)
{
	...
	for (pcl::PointIndices clusterIndex : clusterIndices) {
		/* Create an empty dummy cluster */
		typename pcl::PointCloud<PointT>::Ptr cloud_cluster(new pcl::PointCloud<PointT>);
		/* Loop through all the point indices in a cluster */
		for (int pointIndex : clusterIndex.indices) {
			/* Add points from main cloud to dummy cluster */
			cloud_cluster->points.push_back(cloud->points[pointIndex]);
		}
		cloud_cluster->width = cloud_cluster->size();
		cloud_cluster->height = 1;
		cloud_cluster->is_dense = true;

		/* Push this cloud cluster into clusters vector */
